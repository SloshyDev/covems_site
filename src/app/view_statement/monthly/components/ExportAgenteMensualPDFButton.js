import React, { useState } from "react";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";

// Recibe: agente, recibos, saldoPendiente, year, month
export default function ExportAgenteMensualPDFButton({ agente, recibos, saldoPendiente, year, month }) {
  const [previewUrl, setPreviewUrl] = useState(null);
  const [showModal, setShowModal] = useState(false);
  if (!agente) return null;

  const meses = [
    "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
    "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"
  ];

  const generatePDF = () => {
    const doc = new jsPDF({ orientation: "landscape" });
    const mesNombre = meses[month - 1] || month;
    // Logo COVEMS o fallback
    try {
      const logoBase64 = 'data:image/png;base64,'; // Reemplaza con tu logo en base64
      doc.addImage(logoBase64, 'PNG', 20, 8, 45, 20);
    } catch (error) {
      // Fallback: si hay error con la imagen, usar el círculo azul
      doc.setFillColor(65, 105, 225); // Azul del logo
      doc.circle(35, 20, 15, 'F'); // Círculo azul simulando el logo
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(8);
      doc.setFont('helvetica', 'bold');
      doc.text('COVEMS', 30, 22);
      doc.text('S.C.', 32, 25);
    }

    

    doc.setFontSize(16);
    doc.setTextColor(0, 0, 0);
    doc.setFont('helvetica', 'normal');
    // El logo está en Y=8, altura 20, así que el título debe ir debajo (por ejemplo, Y=32)
    const titleY = 32;
    doc.text(`Estado de Cuenta Mensual - ${mesNombre} ${year}`, 14, 46);

      // Encabezado central de la empresa
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    const empresaText = 'CONSULTORIA EN VENTAS A MERCADOS ESPECIFICOS, S.C.';
    const empresaWidth = doc.getTextWidth(empresaText);
    doc.text(empresaText, (297 - empresaWidth) / 2, 15); // Centrado en página A4 horizontal
    
    doc.setFontSize(12);
    const relacionText = 'RELACION DE POLIZAS ACREDITADAS AL AGENTE';
    const relacionWidth = doc.getTextWidth(relacionText);
    doc.text(relacionText, (297 - relacionWidth) / 2, 25);
    // Info agente y resumen en dos columnas
    // Columna izquierda: info agente
    let leftX = 14;
    // topY debe ir más abajo para mayor separación visual
    let topY = titleY + 28;
    doc.setFontSize(12);
    doc.text(`Clave: ${agente.clave || "-"}`, leftX, topY);
    doc.text(`Nombre: ${agente.nombre || "-"}`, leftX, topY + 7);
    doc.text(`Estado: ${agente.estado || "-"}`, leftX, topY + 14);
    doc.text(`Banco: ${agente.banco || "-"}`, leftX, topY + 21);
    doc.text(`CLABE: ${agente.cuenta_clabe || "-"}`, leftX, topY + 28);

    // Columna derecha: resumen del periodo (mejor visual)
    const rightX = 160;
    const resumenTop = topY;
    const resumenWidth = 90;
    const resumenHeight = 15;
    // Recuadro de fondo suave
    doc.setFillColor(230, 245, 255);
    doc.roundedRect(rightX - 6, resumenTop - 6, resumenWidth, resumenHeight + 12, 4, 4, 'F');
    // Título
    doc.setFontSize(13);
    doc.setTextColor(0, 120, 180);
    doc.text("Resumen del Período", rightX, resumenTop);
    // Totales
    // Verificar si hay recibos anteriores al saldo pendiente
    let usarSaldoPendiente = saldoPendiente;
    if (saldoPendiente && recibos.length > 0) {
      const fechaSaldoPendiente = new Date(saldoPendiente.fecha);
      const hayRecibosAnteriores = recibos.some(recibo => 
        new Date(recibo.fechaMovimiento) < fechaSaldoPendiente
      );
      
      // Si hay recibos anteriores al saldo pendiente, no usar el saldo pendiente
      if (hayRecibosAnteriores) {
        usarSaldoPendiente = null;
      }
    }
    
    // Incluir todos los recibos, incluso los de comisión negativa, y agregar saldo pendiente solo si aplica
    const saldoPendienteValor = usarSaldoPendiente ? Number(usarSaldoPendiente.saldo) || 0 : 0;
    
    const totales = recibos.reduce((acc, r) => {
      acc.primaFracc += Number(r.primaFracc) || 0;
      acc.comisAgente += Number(r.comisAgente) || 0;
      return acc;
    }, { primaFracc: 0, comisAgente: saldoPendienteValor }); // Inicializar comisAgente con el saldo pendiente solo si aplica
    doc.setFontSize(11);
    doc.setTextColor(40, 40, 40);
    doc.text(`Prima Fraccionada: `, rightX, resumenTop + 9);
    doc.setFont('helvetica', 'bold');
    doc.text(`$${totales.primaFracc.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`, rightX + 60, resumenTop + 9, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.text(`Comisión Agente: `, rightX, resumenTop + 17);
    doc.setFont('helvetica', 'bold');
    doc.text(`$${totales.comisAgente.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`, rightX + 60, resumenTop + 17, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    // Tabla movimientos
    let yBanc = topY + 42;
    let rows = [];
    if (usarSaldoPendiente) {
      rows.push([
        usarSaldoPendiente.fecha ? new Date(usarSaldoPendiente.fecha).toLocaleDateString("es-MX") : "-",
        "-", // Poliza
        "-", // Asegurado
        "-", // DSN
        "-", // Prima Fracc.
        usarSaldoPendiente.saldo?.toLocaleString("es-MX", { minimumFractionDigits: 2 }) || "-", // Comis. Agente (usamos saldo)
        usarSaldoPendiente.observaciones || "-"
      ]);
    }
    recibos.forEach(r => {
      rows.push([
        r.fechaMovimiento ? new Date(r.fechaMovimiento).toLocaleDateString("es-MX") : "-",
        r.poliza || "-",
        r.nombreAsegurado || "-",
        r.dsn || "-",
        (r.primaFracc !== null && r.primaFracc !== undefined && r.primaFracc !== "")
          ? Number(r.primaFracc).toLocaleString("es-MX", { minimumFractionDigits: 2 })
          : "-",
        (r.comisAgente !== null && r.comisAgente !== undefined && r.comisAgente !== "")
          ? Number(r.comisAgente).toLocaleString("es-MX", { minimumFractionDigits: 2 })
          : "-",
        r.formaPago || "-"
      ]);
    });

    autoTable(doc, {
      head: [[
        "Fecha Mov.", "Póliza", "Asegurado", "DSN", "Prima Fracc.", "Comis. Agente", "Observaciones"
      ]],
      body: rows,
      startY: yBanc,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [0, 184, 212] },
    });

    return doc;
  };

  const handlePreview = () => {
    const doc = generatePDF();
    const pdfBlob = doc.output("blob");
    const url = URL.createObjectURL(pdfBlob);
    setPreviewUrl(url);
    setShowModal(true);
  };

  const handleDownload = () => {
    const doc = generatePDF();
    const mesNombre = meses[month - 1] || month;
    doc.save(`EstadoCuenta_${agente.clave}_${mesNombre}_${year}.pdf`);
    setShowModal(false);
    if (previewUrl) URL.revokeObjectURL(previewUrl);
  };

  return (
    <>
      <button
        className="bg-cyan-700 hover:bg-cyan-800 text-white font-semibold px-4 py-2 rounded shadow"
        onClick={handlePreview}
        title="Previsualizar PDF mensual"
      >
        Previsualizar PDF Mensual
      </button>
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white rounded-lg shadow-lg max-w-5xl w-full p-4 relative">
            <button
              className="absolute top-2 right-2 text-gray-600 hover:text-red-600 text-2xl font-bold"
              onClick={() => { setShowModal(false); if (previewUrl) URL.revokeObjectURL(previewUrl); }}
              title="Cerrar"
            >
              ×
            </button>
            <h2 className="text-lg font-semibold mb-2 text-gray-800">Previsualización del PDF</h2>
            <div className="w-full h-[70vh] border rounded overflow-hidden mb-4 bg-gray-100">
              {previewUrl && (
                <iframe
                  src={previewUrl}
                  title="Previsualización PDF"
                  className="w-full h-full"
                  frameBorder={0}
                />
              )}
            </div>
            <div className="flex justify-end gap-2">
              <button
                className="bg-cyan-700 hover:bg-cyan-800 text-white font-semibold px-4 py-2 rounded shadow"
                onClick={handleDownload}
              >
                Descargar PDF
              </button>
              <button
                className="bg-gray-400 hover:bg-gray-500 text-white font-semibold px-4 py-2 rounded shadow"
                onClick={() => { setShowModal(false); if (previewUrl) URL.revokeObjectURL(previewUrl); }}
              >
                Cancelar
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
